...
func (s *TicketControllerTestSuite) SetupSuite() {
	...
	s.baseURL = s.server.URL + "/tickets"
}
...
func (s *TicketControllerTestSuite) TestUserLogin_Success() {
...
func (s *TicketControllerTestSuite) TestUserLogin_Failure_WrongPassword() {
	// Ensure that the test user exists in the database before running this test
	var hashedPassword, _ = actions.UserService.HashPassword("testpassword")
	initializations.MySQLDB.Exec("INSERT INTO users (name, email, password) VALUES (?, ?, ?)", "Test User", "testuser@gmail.com", hashedPassword)
...
func (s *TicketControllerTestSuite) TestUserLogin_Failure_UsernameShouldBeEmail() {
	// Ensure that the test user exists in the database before running this test
	var hashedPassword, _ = actions.UserService.HashPassword("testpassword")
	initializations.MySQLDB.Exec("INSERT INTO users (name, email, password) VALUES (?, ?, ?)", "Test User", "testuser@gmail.com", hashedPassword)
...
func (s *TicketControllerTestSuite) TestUserLogin_Failure_UserNotFound() {
	// Ensure that the test user exists in the database before running this test
	var hashedPassword, _ = actions.UserService.HashPassword("testpassword")
	initializations.MySQLDB.Exec("INSERT INTO users (name, email, password) VALUES (?, ?, ?)", "Test User", "testuser@gmail.com", hashedPassword)
